...
def validate_runtime_settings() -> None:
    ...
    if missing:
        msg = "Missing required runtime configuration: " + ", ".join(missing)
        if not (has_file or has_json):
            msg += (
                " | Hosting: set env GOOGLE_CREDENTIALS_JSON to the full service-account "
                "JSON (minify to one line). Local: keep credentials.json and "
                "GOOGLE_CREDENTIALS_FILE=credentials.json, or use GOOGLE_CREDENTIALS_JSON."
            )
